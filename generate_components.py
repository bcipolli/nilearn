...
def generate_components(n_images=np.inf, n_components=20,
                        hemis=('both', 'R', 'L'), random_state=42):
    ...
    for hemi in hemis:
        ...
        # Write to disk
        out_path = op.join('ica_nii', str(n_components),
                           '%s_ica_components.nii.gz' % hemi)
        ...
        for idx, (ic, ic_terms) in enumerate(zip(ica_maps, ica_terms)):
            ...
            out_path = op.join('ica_maps', str(n_components),
                               '%s_component_%i.png' % (hemi, idx))
            if not op.exists(op.dirname(out_path)):
                os.makedirs(op.dirname(out_path))
...
if __name__ == '__main__':
    ...
    generate_components(n_components=20)
